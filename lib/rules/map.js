...
function isFixed(node) {
  ...
  if (consequent.type !== 'CallExpression' || !consequent.callee || consequent.callee.type !== 'MemberExpression') {
    return false;
  }

  return consequent.callee.object.name === node.arguments[0].name && consequent.callee.property.name === 'map';
}
  ...
  create: context => {
    ...
    return {
      ...
      CallExpression: node => {
        ...
        if (node.callee.object.name === '_' && node.callee.property.name === 'map' && !isFixed(node)) {
